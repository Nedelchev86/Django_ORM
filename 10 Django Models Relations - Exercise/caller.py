...
def calculate_average_rating_for_product_by_name(product_name: str):
    ...
    # avv_rating = sum(x.rating for x in product.review_set.all())/product.review_set.count()
    reviews = product.reviews.all()
    avv_rating = sum(x.rating for x in reviews)
    return avv_rating / len(reviews)
...
def calculate_licenses_expiration_dates():
    ...
    return "\n".join(
        [f"License with id: {licenses.license_number} expires on {licenses.issue_date + timedelta(days=365)}!" for licenses in all_licenses]
    )

def get_drivers_with_expired_licenses(due_date):
    result = []
    for driver in Driver.objects.all():
        if driver.drivinglicense.issue_date + timedelta(days=365) > due_date:
