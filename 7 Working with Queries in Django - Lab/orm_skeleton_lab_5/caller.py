...
def find_authors_nationalities():
    # return "\n".join([f"{author} is {author.nationality}" for author in Author.objects.filter(nationality__isnull=False)])
    return "\n".join([f"{author} is {author.nationality}" for author in Author.objects.exclude(nationality=None)])


def order_books_by_year():
    return "\n".join([f"{book.publication_year} year: {book}" for book in Book.objects.order_by("publication_year", "title")])
...
def delete_review_by_id(review_id):
    ...
    # info = "".join([f"{str(r)} was deleted" for r in review])
    # review.delete()
    # return info
...
def filter_authors_by_nationalities(nationality):
    ...
    return "\n".join([author.biography if author.biography else str(author) for author in Author.objects.filter(nationality=nationality).order_by("first_name", "last_name")])


# def filter_authors_by_birth_year(year1, year2):
#     return "\n".join([f"{author.birth_date}: {str(author)}" for author in Author.objects.filter(birth_date__year__gte=year1, birth_date__year__lte=year2).order_by("-birth_date")])


def filter_authors_by_birth_year(year1, year2):
    return "\n".join([f"{author.birth_date}: {str(author)}" for author in Author.objects.filter(birth_date__year__range=(year1, year2)).order_by("-birth_date")])
